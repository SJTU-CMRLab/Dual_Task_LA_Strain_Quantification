...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        [B, C, T, W, L] = x1.shape
        spacialposembed = self.spacialposembed
        temporalposembed = self.temporalposembed
        
        [_,target_x,target_y] = rearrange(x2, 'B C T W L -> (B T) (W L) C').shape
        if not (self.spacialposembed.shape[1] == target_x):
            spacialposembed = self.spacialposembed.unsqueeze(1)
            spacialposembed = F.interpolate(spacialposembed, size=(target_x, target_y), mode='bicubic', align_corners=False)
            spacialposembed = spacialposembed.squeeze(1)

        rearranged_x1 = rearrange(x1, 'B C T W L -> (B T) (W L) C') + spacialposembed
        rearranged_x2 = rearrange(x2, 'B C T W L -> (B T) (W L) C') + spacialposembed
        ...
        [_, target_x, target_y] = rearrange(x2, 'B C T W L -> (B W L) T C').shape
        if not (self.temporalposembed.shape[1] == target_x):
            temporalposembed = self.temporalposembed.unsqueeze(1)
            temporalposembed = F.interpolate(temporalposembed, size=(target_x, target_y), mode='bicubic', align_corners=False)
            temporalposembed = temporalposembed.squeeze(1)

        rearranged_x1 = rearrange(x1, 'B C T W L -> (B W L) T C') + temporalposembed
        rearranged_x2 = rearrange(x2, 'B C T W L -> (B W L) T C') + temporalposembed
